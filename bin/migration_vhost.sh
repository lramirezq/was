...
#export virtual host  to file ini
echo "Starting export of virtual host to file"
sudo $dmgr_was_source/bin/wsadmin.sh -username $user_was_source -password $pass_was_source -lang jython -f export_vhosts.py  $conf_vhosts $folder_properties 
wait
echo "=========================================================="
...
echo "importar virtual hosts en destino"
file_details=$folder_properties/vhosts_export.ini
echo "Archivo con config : [$file_details]"|
#ssh $ip_target "cd $work_dir_target/bin && sudo $dmgr_was_target/bin/wsadmin.sh -username $user_was_target -password $pass_was_target -lang jython -f $work_dir_target/bin/import_vhosts.py $file_details"
wait
