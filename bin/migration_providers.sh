...
echo "Starting export of providers to file"
sudo $dmgr_was_source/bin/wsadmin.sh -username $user_was_source -password $pass_was_source -lang jython -f export_providers.py $usuario $folder_properties $folder_bin $conf_providers $ip_target
wait
echo "=========================================================="
...
echo "importar providers  en destino"
file_details=$folder_properties/providers_export.ini
echo "Archivo con config : [$file_details]"|
#ssh $ip_target "cd $work_dir_target/bin && sudo $dmgr_was_target/bin/wsadmin.sh -username $user_was_target -password $pass_was_target -lang jython -f $work_dir_target/bin/import_providers.py $file_details"
wait
